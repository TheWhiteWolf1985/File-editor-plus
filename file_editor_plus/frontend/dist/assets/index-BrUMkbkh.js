...
`),n=Math.max(1,o.length),l=Math.max(1,o[o.length-1].length+1);(n!==this.cursorLine||l!==this.cursorCol)&&(this.cursorLine=n,this.cursorCol=l,this.lastCursorLine=n,this.lastCursorCol=l,console.debug("[app-root] cursor",{pos:t,line:n,col:l})),this.updateActiveIndentSegment(t,s)}updateCursorFromTextarea(){if(!this.editorRef)return;const t=this.editorRef,e=t.selectionStart??0;this.updateCursorFromPos(e,t.value)}syncEditorOverlay(){this.editorRef&&this.syncScroll({target:this.editorRef})}syncBaseOverlay(){this.basePreRef&&this.syncBaseScroll({target:this.basePreRef})}jumpToPosition(t,e){if(!this.editorRef)return;const s=this.editorRef,i=Math.max(1,Math.min(t,this.content.split(`
